...
(async () => {
    ...
    if (saveResultBtn) {
        saveResultBtn.addEventListener('click', () => exportResults());
    }
    ...
    function displayResults(morph, matches, landmarks, info, img) {
        ...
        // Show save button for new analysis
    ...
    `;
      ...
      `;
        ...
        HistoryManager.save(morph, matches, info, img);
    }
    ...
    async function exportResults() {
        ...
        const [morph, matches, landmarks, info, img] = lastAnalysisData;
        const btn = document.getElementById('saveResultBtn');
        const originalText = btn.innerHTML;

        try {
            btn.disabled = true;
            btn.innerHTML = `<span class="btn-icon">⏳</span> ${t('downloading')}`;

            // Populate Export Card
            document.getElementById('exportDate').textContent = new Date().toLocaleDateString(currentLang, { year: 'numeric', month: 'long', day: 'numeric' });

            const userImg = document.getElementById('exportUserImage');
            userImg.src = img.src || img.toDataURL(); // Handle both Image element and Canvas

            // Draw overlay on export card
            const overlay = document.getElementById('exportFaceOverlay');
            // We need to wait for image to load in the hidden div if it's a new src
            await new Promise(r => setTimeout(r, 100));

            // Draw simplified landmarks on overlay
            const ovCtx = overlay.getContext('2d');
            overlay.width = userImg.naturalWidth || userImg.width;
            overlay.height = userImg.naturalHeight || userImg.height;
            ovCtx.clearRect(0, 0, overlay.width, overlay.height);

            // Draw simple face oval
            if (landmarks) {
                ovCtx.strokeStyle = 'rgba(6, 214, 160, 0.6)';
                ovCtx.lineWidth = 3;
                const faceOval = [10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288, 397, 365, 379, 378, 400, 377, 152, 148, 176, 149, 150, 136, 172, 58, 132, 93, 234, 127, 162, 21, 54, 103, 67, 109, 10];
                ovCtx.beginPath();
            ...
            const statsToShow = [
                { label: 'Face Shape', val: morph?.features?.faceShape?.valueKey },
                { label: 'Nose Type', val: morph?.features?.noseType?.valueKey },
                { label: 'Facial Index', val: morph?.indices?.facialIndex?.value },
                { label: 'Nasal Index', val: morph?.indices?.nasalIndex?.value },
                { label: 'Jaw Type', val: morph?.features?.jawType?.valueKey },
                { label: 'Eye Shape', val: morph?.features?.eyeShape?.valueKey }
            ];

            statsToShow.forEach(s => {
                if (s.val) {
                    // Try to translate value if it's a key, else use as is
                    let displayVal = s.val;
                    if (s.val.startsWith('val_')) displayVal = t(s.val, { val: '' }).split('—')[0].trim(); // Get main label part

                    const div = document.createElement('div');
                    div.className = 'export-stat-item';
                    div.innerHTML = `
                        <div class="export-stat-label">${s.label}</div>
                        <div class="export-stat-value">${displayVal}</div>
                    `;
            ...
            link.download = `PhenoType-Analysis-${Date.now()}.png`;
    ...
    const HistoryManager = (() => {
        ...
        async function save(morph, matches, info, imgElement) {
            const history = getHistory();
            const topMatch = matches[0];

            // Create thumbnail
            const thumb = await createThumbnail(imgElement);

            const item = {
                id: Date.now(),
                date: new Date().toISOString(),
                info: info,
                topMatchName: topMatch ? topMatch.name : 'Unknown',
                topMatchScore: topMatch ? topMatch.score : 0,
                thumb: thumb,
                // We don't save full morph/matches data to save space,
                // but we could save a summary if needed for reloading.
                // For now, reloading will just show the summary from history.
                // To support full reload, we'd need to re-run analysis or save more data.
                // Let's save essential data to reconstruct a basic view.
                summaryData: {
                    morphFeatures: morph ? morph.features : null,
                    morphIndices: morph ? morph.indices : null,
            ...
            try {
                ...
                // Show toast or log
                console.log('[History] Saved analysis.');
                // Update UI if open (handled by render)
            } catch (e) {
                console.warn('[History] Storage full or error:', e);
            }
        }
        ...
        function load(item) {
            ...
            // Restore UI to results view (partial restore since we don't have full data/image)
            ...
            // Hide save button for history items (data is incomplete for high-res export)
            const saveBtn = document.getElementById('saveResultBtn');
            ...
            // 1. Basic Info
            const genderLabel = item.info.gender === 'male' ? t('genderMale') : t('genderFemale');
            const ageRange = formatAgeRange(item.info.age);
            document.getElementById('genderAgeInfo').innerHTML = `
              <div class="info-item">
                <div class="info-label">${t('label_gender')}</div>
                <div class="info-value">${genderLabel} ${item.info.gender === 'male' ? '♂️' : '♀️'}</div>
              </div>
              <div class="info-item">
                <div class="info-label">${t('confidence')}</div>
                <div class="info-value">${item.info.genderProb}%</div>
              </div>
              <div class="info-item">
                <div class="info-label">${t('estAge')}</div>
            ...
            // 2. Morphology (if saved)
            if (item.summaryData.morphFeatures) {
                let morphHTML = '';
                for (const [key, feat] of Object.entries(item.summaryData.morphFeatures)) {
                    const label = t(feat.label);
                    const val = t(feat.valueKey, feat.params);
                    const detail = t(feat.detailKey, feat.params);
                    morphHTML += `
            ...
            if (item.summaryData.morphIndices) {
                let indicesHTML = '';
                for (const [key, idx] of Object.entries(item.summaryData.morphIndices)) {
                    const label = t(idx.label);
                    const desc = t(idx.descKey, idx.params);
                    indicesHTML += `
                      <div class="index-item">
            ...
            // 3. Phenotype Matches
            if (item.summaryData.matches) {
                 let phenoHTML = '';
                item.summaryData.matches.forEach((m, idx) => {
                    const primarySrc = m.isBasic
                        ? PhenotypeMatcher.getBasicImagePath(m.name, m.sex)
                        : PhenotypeMatcher.getImagePath(m.name, m.sex);
                    const fallbackSrc = m.isBasic
            ...
            // 4. Face Map (Can't fully restore without original image, show placeholder or user thumbnail)
            const canvas = faceMapCanvas;
            const ctx = canvas.getContext('2d');
            canvas.width = 300;
            canvas.height = 300;
            ...
            img.onload = () => {
                ...
                ctx.fillText(t('savedToHistory'), 150, 150);
            };
            img.src = item.thumb;

            // Set lastAnalysisData to allow language toggle (partially)
            // We don't have full data, so language toggle might not work perfectly for history items
